...
<p style="text-align: justify;">
  A <strong>view</strong> is a version of a customized list or form (Layout Configuration) which defines whch fields appear and in what order. User with the <strong>admin</strong> or <strong>personalize_list</strong> role can add or remove columns from a list for <i>all users</i>.
</p>
